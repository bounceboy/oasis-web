...
async function ekstrakGambar(buf: Buffer, filename: string): Promise<string> {
  // OASIS Local: baca gambar via model vision Ollama (perlu model multimodal,
  // mis. `ollama pull qwen2.5vl:7b`). Jika model tidak tersedia, gambar dilewati.
  ...
  try {
    const res = await fetch(`${ollamaUrl}/api/chat`, {
      ...
      body: JSON.stringify({
        ...
        messages: [{
          role: 'user',
          content: 'Ekstrak semua teks, angka, dan informasi penting dari gambar ini dalam format teks biasa.',
          images: [base64],
        }],
      }),
    })
    ...
    if (!res.ok) {
      return `[Gambar ${filename} dilewati — model vision ${visionModel} tidak tersedia di Ollama]`
    }

    const json = await res.json() as { message?: { content?: string } }
    return json.message?.content ?? ''
  } catch {
    return `[Gambar ${filename} dilewati — Ollama tidak dapat dihubungi]`
  }
}
