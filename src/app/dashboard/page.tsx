...
export default async function DashboardPage() {
  ...
  // Admin/supervisor lihat semua, pemeriksa hanya miliknya
  let query = db()
    .from('pemeriksaan_sessions')
    .select('id, nama_entitas, jenis_pemeriksaan, status, created_at, dokumen_nama, user_id')
    .order('created_at', { ascending: false })
    .limit(50)

  if (user.role === 'pemeriksa') {
    query = query.eq('user_id', user.id) as typeof query
  } else if (user.role === 'supervisor' && user.direktorat_id) {
    query = query.eq('direktorat_id', user.direktorat_id) as typeof query
  }
    ...
    <div className="min-h-screen bg-slate-950 text-white">
      <nav className="border-b border-slate-800 px-6 py-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          {user.role === 'admin' && (
            <Link href="/admin" className="text-xs px-3 py-1 rounded-full bg-purple-900/30 text-purple-400 hover:bg-purple-900/50 transition-colors">
              Admin Panel
            </Link>
          )}
      ...
      <div className="max-w-5xl mx-auto px-6 py-8">
        <div className="flex items-center justify-between mb-6">
          ...
          <Link href="/pemeriksaan" className="bg-blue-600 hover:bg-blue-500 text-white px-4 py-2 rounded-lg text-sm font-medium transition-colors">
            + Pemeriksaan Baru
          </Link>
        </div>
        ...
        {(!sessions || sessions.length === 0) ? (
          ...
          <div className="space-y-2">
            {sessions.map((s) => (
              <Link key={s.id} href={`/pemeriksaan/${s.id}`}
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    ...
                    <span className="text-slate-600 text-xs">{new Date(s.created_at).toLocaleDateString('id-ID')}</span>
                  </div>
                </div>
              </Link>
            ))}
